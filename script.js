...
function createPropertyCard(property) {
    ...
    if (property.parking) {
        metaHTML += `<div class="meta-item">🅿️ Parking</div>`;
    }
    if (property.furnished) {
        metaHTML += `<div class="meta-item">🛋️ ${property.furnished}</div>`;
    }
    if (property.gender) {
        metaHTML += `<div class="meta-item">👥 ${property.gender} Only</div>`;
    }
    if (property.food !== undefined) {
        metaHTML += `<div class="meta-item">🍽️ ${property.food ? 'Food Included' : 'No Food'}</div>`;
    }
    ...
    card.innerHTML = `
        ...
        <div class="listing-content">
            ...
            <div style="display: flex; gap: 8px; margin-top: 12px;">
                ...
                <button class="btn btn-outline" style="padding: 12px; font-size: 0.9rem;" onclick="showOnMap(${property.lat}, ${property.lng})" title="View on Map">🗺️</button>
            </div>
        </div>
    `;
